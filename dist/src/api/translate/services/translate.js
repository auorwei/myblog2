...
const StrapiApiService_1 = __importDefault(require("../../../admin/services/StrapiApiService"));
const TranslationService_1 = require("../../../admin/services/TranslationService");
const HtmlParseService_1 = require("../../../admin/services/HtmlParseService");
exports.default = strapi_1.factories.createCoreService('api::translate.translate', ({ strapi }) => ({
    /**
     * 获取支持的语言列表
     */
    async getLocales() {
        try {
            ...
            const apiService = new StrapiApiService_1.default(baseUrl, token);
    ...
    async translateContent(entryId, apiEndpoint, targetLocale = null) {
        ...
        try {
            ...
            const apiService = new StrapiApiService_1.default(baseUrl, token);
            const translationService = new TranslationService_1.TranslationService(authKey);
            // 获取英文内容
            ...
            const category = (_a = entry.category) === null || _a === void 0 ? void 0 : _a.documentId;
            const tags = entry.tags;
            const slug = entry.slug;
            const coverPicture = (_b = entry.coverPicture) === null || _b === void 0 ? void 0 : _b.id;
            // 获取所有可用语言
            const locales = await apiService.getAllLocales();
            ...
            if (category) {
                ...
                if (categoryEntry) {
                    ...
                    const missingCategoryLocalesSet = targetLocales.filter(locale => locale !== 'en' && !categoryLocalesList.includes(locale));
                    if (missingCategoryLocalesSet.length > 0) {
                        for (const missingLocale of missingCategoryLocalesSet) {
                            if (missingLocale === 'en')
                                continue;
                            const categoryTranslation = await translationService.translate(`${categoryName}<br>${categoryDescription}`, missingLocale);
                            ...
                            const categoryRes = await apiService.publishEntry('categories', category, missingLocale, categoryPayload, false);
            ...
            if (tags && tags.length > 0) {
                for (const tag of tags) {
                    ...
                    if (tagEntry) {
                        ...
                        const missingTagLocalesSet = targetLocales.filter(locale => locale !== 'en' && !tagLocalesList.includes(locale));
                        if (missingTagLocalesSet.length > 0) {
                            for (const missingLocale of missingTagLocalesSet) {
                                if (missingLocale === 'en')
                                    continue;
                                const tagTranslation = await translationService.translate(`${tagName}<br>${tagDescription}`, missingLocale);
                                ...
                                const tagRes = await apiService.publishEntry('tags', tag.documentId, missingLocale, tagPayload);
            ...
            for (const locale of targetLocales) {
                ...
                const [stripped, attrs] = HtmlParseService_1.HtmlParseService.decompose(articleHtml);
                // 翻译内容
                const translatedHtml = await translationService.translate(stripped, locale);
                // 重新组合HTML
                const recomposeHtml = HtmlParseService_1.HtmlParseService.recompose(translatedHtml, attrs);
                const finalHtml = HtmlParseService_1.HtmlParseService.cleanForStrapi(recomposeHtml);
                // 分离标题和内容
