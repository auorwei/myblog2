...
export class HtmlParseService {
   ...
   */
  static decompose(html: string): [string, Array<[string, string, string, boolean]>] {
    const tokens = html.split(/(<[^>]+>)/);
    const attrList: Array<[string, string, string, boolean]> = [];
   ...
   * @returns 重组后的HTML
   */
  static recompose(translatedHtml: string, attrList: Array<[string, string, string, boolean]>): string {
   ...
   * @returns 清理后的HTML
   */
  static cleanForStrapi(html: string): string {
